...
typedef enum {TEST_BB = 0x0, TEST_PID = 0x01, TEST_RESERVED = 0x02, 
				TEST_CHARACTERIZE = 0x03, TEST_INVALID = 0xFF} Test_t;
				
typedef enum {SLCT_OFFSET, SLCT_GP, SLCT_GD, SLCT_GI, SLCT_INVALID}  ParamSel_t;
...
int main()
{
	...
	while (1)
	{
		...
		if (wrlcd_dynamic)
		{
			...
			switch (param_select)
			{
				case SLCT_OFFSET:	row = 2;	col = 13;	break;
				case SLCT_GP:		row = 1;	col = 3;	break;
				case SLCT_GD:		row = 1;	col = 13;	break;
				case SLCT_GI:		row = 1;	col = 8;	break;		
				case SLCT_INVALID:	break;
			}
		...
		if (rotcnt != old_rotcnt)
		{
			...
			{
				setpoint += (rotcnt - old_rotcnt) * ROTCNT_DELTA * ROTCNT_SPEEDUP_FACTOR;
			}
			else
			{
		...
		if(diff_btnsw & BTN_RUN_XFER)	// run a test
		{
			...
			if (funcPtr != NULL)
			{
 	 			...
 	 			for (smpl_idx = 2; smpl_idx < n; smpl_idx++)
				{
					...
					voltstostrng(vSP, s2); 
					xil_printf("%d\t%d\t%s\t%s\t%d\n\r", smpl_idx, count, s1, s2, PIDdebug[smpl_idx].pwm_count);				
					LCD_setcursor(2, 2);
					LCD_wrstring("   ");
					LCD_setcursor(2, 2);
